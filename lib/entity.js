  ...
  return p.do(
    function () {
      ...
      return ohm.exec(clazz.isObject ? 'hgetall' : 'get', k)
        .then(function (result) {
          if (!result) {
            e.throwNotFound(util.format('entity "%s" #%s not found', type, id), {id: id});
          }
          if (!clazz.isObject) {
      ...
      return entity.iterateIndexes(function (index, name, k) {
        ...
        return ohm.exec('exists', k).then(function (result) {
          ...
          if (result) {
            ...
            e.throwConflict(util.format('%s "%s" already exists', name, nameValue), {name: name, value: nameValue});
          }
        });
      });
    },
        ...
        return p.do(
          function checkLinkToEntity() {
            return p.do(
              ...
              function (result) {
                if (result) {
                  e.throwConflict(util.format('%s "%s" already exists', name, value), {name: name, value: value});
                }
              });
          },
  ...
  return p.do(
    function () {
      ...
      if (typeof id === 'undefined') {
        e.throwBadFormat('entity "%s" has no %s property', type, id, {type: type, name: 'id'});
      }
      type = entity.type;
      clazz = entity.getClass();
    ...
    function check() {
      ...
      return p.do(function check() {
        return ohm.validateSchema(entity.value, type, 'new')
          .catch(function (err) {
            e.throwBadFormat({schemaErrors: err});
          });
      });
    },
    function generateId() {
