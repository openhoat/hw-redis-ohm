...
Entity.delete = function (id) {
  var clazz = this
    , type = clazz.type;
  logger.enabledLevels.info && log.info('deleting "%s" entity from ID #%s', type, id);
  return ohm.getModelClass(type).load(id)
    .then(function (result) {
      var entity = ohm.create(type, result);
  ...
  return clazz.findIdsByIndex(name, value)
    .then(function (result) {
      return p.map(result, clazz.load.bind(clazz))
        .then(function (result) {
          return _.sortBy(result, sort || ohm.getSchemaId(type));
        });
    });
};
...
Entity.load = function (id) {
  ...
  return p.do(
    ...
    function () {
      return ohm.filterProperties(entity.value, type, 'get');
    });
};
...
Entity.prototype.getId = function () {
  return this.value[ohm.getSchemaId(this.type)];
};

Entity.prototype.getClass = function () {
  return ohm.getModelClass(this.type);
};
...
Entity.prototype.update = function () {
  ...
  return p.do(
    ...
    function () {
      return clazz.load(id)
        .then(function (result) {
          existing = ohm.create(type, result);
        });
    },
    function removeRefs() {
    ...
    function saveEntity() {
      ...
      return ohm.exec(clazz.isObject ? 'hmset' : 'set', k, data)
        ...
        .then(function () {
          return ohm.filterProperties(result, type, 'get');
        });
    });
};
