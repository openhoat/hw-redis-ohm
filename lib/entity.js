...
class Entity {
  ...
  static findOneByIndex(name, value, sort) {
    const clazz = this;
    return clazz.findByIndex(name, value, sort).then(result => _.head(result));
  }
  ...
  static saveOrUpdate(value, multi) {
    const clazz = this;
    const type = this.type;
    const entity = clazz.create(_.omit(value, 'ttl'));
    if (value.ttl) {
      entity.ttl = value.ttl;
    }
        ...
        return entity.iterateIndexes((index, name, k) => {
          ...
          return ohm.exec('exists', k)
            .then(result => result && ohm.exec('get', k)
              .then(result => {
                if (result !== id) {
                  const names = name.indexOf(',') !== -1 ? name.split(',') : name;
                  const nameValues = Array.isArray(names) ? names.map(item => entity.value[item]) : entity.value[name];
                  ohm.e.throwEntityConflict({type, attrName: names, attrValue: nameValues});
                }
              })
            );
        });
