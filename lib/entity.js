...
class Entity {
  ...
  static findByIndex(name, value, sort) {
    ...
    return clazz.findIdsByIndex(name, value)
      .then(result => p.map(result, clazz.load.bind(clazz))
    ...
    return p.do(
      () => {
        ...
        return ohm.exec('keys', ohm.toHash(type, '*'))
          .then(keys => p.map(keys, k => {
            const id = _.last(k.split(':'));
            return clazz.load(id);
          }));
      },
      result => _.sortBy(result, 'value.' + (sort || ohm.getSchemaId(type))));
  }
  ...
  static update(value) {
    const clazz = this;
    return p.do(() => clazz.create(value).update());
  }
  ...
  iterateIndexes(iterator) {
    ...
    return p.do(() => {
      ...
      return p.forIn(clazz.indexes, (index, name) => {
        if (typeof index.getValue === 'function') {
          return p.do(
            () => index.getValue(entity)
  ...
  iterateLinks(withValue, iterator) {
    ...
    return p.do(() => {
      ...
      return p.forIn(clazz.links, (link, name) => {
        const value = entity.value[name];
        if (withValue) {
          if (typeof value === 'undefined' || value === null || (Array.isArray(value) && !value.length)) {
            return;
          }
          return p.each(Array.isArray(value) ? value : [value], value => {
  ...
  loadLinks() {
    ...
    return p.do(() => {
      ...
      return entity.iterateLinks(false, (link, name, k) => p
        .do(() => ohm.exec(link.unique ? 'get' : 'smembers', k)
          .then(result => {
  ...
  removeLinks(multi) {
    ...
    return p.do(() => {
      ...
      return entity
        .iterateLinks((link, name, k, reverseK, value) => p.do(
          () => {
            if (link.reverseUnique) {
              return ohm.execMulti(localMulti, 'del', k);
            } else {
  ...
  update(multi) {
    ...
    return p.do(
      ...
      () => existing.removeLinks(localMulti)
      ...
      () => {
        ...
        let data = ohm.toRedis(_.omit(result, ignoreProps), type);
        if (typeof data === 'object') {
          data = _(data).omitBy(_.isNull).omitBy(_.isUndefined).value();
        }
        return p.do(
          () => ohm.execMulti(localMulti, clazz.isObject ? 'hmset' : 'set', k, data),
          () => {
            if (entity.ttl) {
              return ohm.execMulti(localMulti, 'expire', k, entity.ttl);
            }
