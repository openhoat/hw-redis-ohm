  ...
  return p.do(
    function () {
      ...
      return ohm.exec(clazz.isObject ? 'hgetall' : 'get', k)
        .then(function (result) {
          if (!result) {
            ohm.e.throwEntityNotFound({type: type, attrName: 'id', attrValue: id});
          }
          if (!clazz.isObject) {
      ...
      return entity.iterateIndexes(function (index, name, k) {
        ...
        return ohm.exec('exists', k).then(function (result) {
          ...
          if (result) {
            ...
            ohm.e.throwEntityConflict({type: type, attrName: name, attrValue: nameValue});
          }
        });
      });
    },
        ...
        return p.do(
          function checkLinkToEntity() {
            return p.do(
              ...
              function (result) {
                if (result) {
                  ohm.e.throwEntityConflict({type: type, attrName: name, attrValue: value});
                }
              });
          },
  ...
  return p.do(
    function () {
      ...
      if (typeof id === 'undefined') {
        ohm.e.throwEntityValidation({type: type, attrName: 'id', attrValue: id});
      }
      clazz = entity.getClass();
        ...
        return p.do(
          function () {
            return index.getValue(entity)
              .catch(ohm.e.EntityNotFoundError, function () {
              });
          },
          function (k) {
    ...
    function check() {
      ...
      return p.do(function check() {
        return ohm.validateSchema(entity.value, type, 'new')
          .catch(function (err) {
            throw new ohm.errors.EntityValidationError(type, {schemaErrors: err});
          });
      });
    },
    function generateId() {
