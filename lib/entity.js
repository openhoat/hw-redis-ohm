  ...
  return p.do(
    function () {
      ...
      return ohm.exec(clazz.isObject ? 'hgetall' : 'get', k)
        .then(function (result) {
          if (!result) {
            throw new ohm.errors.EntityNotFoundError(type, 'id', id);
          }
          if (!clazz.isObject) {
      ...
      return entity.iterateIndexes(function (index, name, k) {
        ...
        return ohm.exec('exists', k).then(function (result) {
          ...
          if (result) {
            ...
            throw new ohm.errors.EntityConflictError(type, name, nameValue);
          }
        });
      });
    },
        ...
        return p.do(
          function checkLinkToEntity() {
            return p.do(
              ...
              function (result) {
                if (result) {
                  throw new ohm.errors.EntityConflictError(type, name, value);
                }
              });
          },
  ...
  return p.do(
    function () {
      ...
      if (typeof id === 'undefined') {
        throw new ohm.errors.EntityValidationError(type, 'id', id);
      }
      clazz = entity.getClass();
