...
errors = {
  OhmError: e.helpers.generateClass('OhmError', {
    generateMessage: function () {
      return util.format('ohm error');
    }
  })
};
_.extend(errors, {
  UnsupportedOhmError: e.helpers.generateClass('UnsupportedOhmError', {
    extends: errors.OhmError,
    args: ['cmd'],
    generateMessage: function () {
      return util.format('unsupported operation "%s" error', this.cmd);
    }
  }),
  RedisOhmError: e.helpers.generateClass('RedisOhmError', {
    extends: errors.OhmError,
    args: ['redisError'],
    generateMessage: function () {
      return util.format('redis error "%s"', this.redisError);
    }
  }),
  EntityError: e.helpers.generateClass('EntityError', {
    extends: errors.OhmError,
    args: ['type'],
    generateMessage: function () {
      return util.format('entity "%s" error', this.type);
    }
  })
});
_.extend(errors, {
  EntitySchemaNotFoundError: e.helpers.generateClass('EntitySchemaNotFoundError', {
    extends: errors.EntityError,
    args: ['schema'],
    generateMessage: function () {
      return util.format('entity schema "%s" not found', this.schema);
    }
  }),
  EntityConflictError: e.helpers.generateClass('EntityConflictError', {
    extends: errors.EntityError,
    args: ['type', 'attrName', 'attrValue'],
    generateMessage: function () {
      return util.format('entity "%s" conflict for "%s" with value "%s"', this.type, this.attrName, this.attrValue);
    }
  }),
  EntityNotFoundError: e.helpers.generateClass('EntityNotFoundError', {
    extends: errors.EntityError,
    args: ['type', 'attrName', 'attrValue'],
    generateMessage: function () {
      return util.format('entity "%s" not found for "%s" with value "%s"', this.type, this.attrName, this.attrValue);
    }
  }),
  EntityValidationError: e.helpers.generateClass('EntityValidationError', {
    extends: errors.EntityError,
    args: ['type', 'attrName', 'attrValue'],
    generateMessage: function () {
      return util.format('entity "%s" validation failed for "%s" with value "%s"', this.type, this.attrName, this.attrValue);
    }
  })
});
...
ohm = {
  ...
  exec: function (cmd, k, values) {
    ...
    return p.do(function () {
      ...
      if (typeof ohm.cli[cmd + 'Async'] !== 'function') {
        throw new errors.UnsupportedOhmError(cmd);
      }
      ...
      return promise.catch(function (err) {
        ...
        throw new errors.RedisOhmError(err);
      });
    });
  },
  ...
  getSchema: function (namespace, type, op) {
    ...
    if (!schema) {
      throw new errors.EntitySchemaNotFoundError(_.compact(arguments).join('/'));
    }
    return schema;
  },
