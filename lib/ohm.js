...
ohm = {
  ...
  init: function (opt) {
    ...
    if (ohm.config.roHost) {
      ...
      ohm.roCliAsync = p.promisifyAll(ohm.roCli);
    }
  ...
  initSchemas: function () {
    ...
    _.forIn(ohm.config.schemas, function (schemaSpec, schemaName) {
      ...
      logger.enabledLevels.trace && log.trace('schema "%s" :', schemaName, JSON.stringify(schema, null, 2));
    });
  ...
  start: function (opt, cb) {
    ...
    return ohm.cliAsync.selectAsync(ohm.config.db)
      .then(function () {
        if (ohm.roCliAsync) {
          return ohm.roCliAsync.selectAsync(ohm.config.db);
        }
      })
      .then(function () {
        ohm.started = true;
    ...
    return p.do(
      function () {
        ...
        return (typeof ohm.cliAsync.quitAsync === 'function' ? ohm.cliAsync.quitAsync() : p.resolve())
          .then(function () {
            if (ohm.roCliAsync) {
              return (typeof ohm.roCliAsync.quitAsync === 'function' ? ohm.roCliAsync.quitAsync() : p.resolve());
            }
          })
          .then(function () {
            ohm.started = false;
