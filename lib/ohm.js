...
const ohm = {
  config: {
    ...
    errors: [['OHM', {
      customizeClass: function customizeClass() {
      ...
      constructor: function EntityValidationError() {
        if (_.get(this, 'extra.schemaErrors')) {
          this.message = util.format('entity "%s" validation failed because of schema error "%s"', this.extra.type, _.map(this.extra.schemaErrors, 'desc').join(','));
        } else if (_.get(this, 'extra.attrName')) {
          this.message = util.format('entity "%s" validation failed for "%s" with value "%s"', this.extra.type, this.extra.attrName, this.extra.attrValue);
        } else {
  ...
  init: (opt = {}) => {
    ...
    e.initErrors(ohm.config.errors);
    ohm.initSchemas();
