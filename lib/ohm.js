...
ohm = {
  ...
  modelClasses: {},
  create: function (schemaName, opt) {
    var clazz, entity;
    clazz = ohm.getModelClass(schemaName);
    entity = new clazz(opt);
    return entity;
  },
    ...
    if (logger.enabledLevels.redis) {
      promise = promise
        .then(function (result) {
          log.redis('%s %s%s :', cmd.toUpperCase(), k, values ? ' ' + JSON.stringify(values) : '', result);
          return result;
        })
  ...
  getModelClass: function (schemaName) {
    if (!ohm.modelClasses[schemaName]) {
      ohm.modelClasses[schemaName] = ohm.createEntityClass(schemaName);
    }
    return ohm.modelClasses[schemaName];
  },
  ...
  init: function (opt) {
    ...
    _.forIn(opt.schemas, function (schemaSpec, name) {
      var schemaName, defaultSchema, schema, schemaId, linkNames, dbDefaults;
      ...
      _.defaultsDeep(schema[ohm.config.schemaMetaPrefix].operations.db, dbDefaults);
      _.forIn(schema[ohm.config.schemaMetaPrefix].operations, function (value, type) {
  ...
  setSchemaDefaults: function (data, namespace, type, mode) {
    var schema;
    if (typeof data !== 'object') {
      return;
    }
    namespace = _.camelCase(namespace);
    schema = ohm.getSchema(namespace, type, mode);
    if (!schema) {
      logger.enabledLevels.debug && log.debug('schema for "%s/%s/%s" not found : ignore defaults', namespace, type, mode);
      return data;
    }
    logger.enabledLevels.debug && log.debug('setting defaults with schema "%s/%s/%s"', namespace, type, mode);
  ...
  toHash: function (o) {
    ...
    result = _.compact(hashes.map(function (item) {
      if (Array.isArray(item)) {
        return item/*.map(_.kebabCase)*/.join(':');
      }
      return item;//index < hashes.length - 1 ? _.kebabCase(item) : item;
    })).join(':');
    return result;
  },
